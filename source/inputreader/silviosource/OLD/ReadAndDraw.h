 ...
 * Author: silvio14
 *
 * Created on July 24, 2017, 4:47 PM
 */
...
int SilvioFunction() {
    ...
    vector <Reactor_unit> Reactor(50);         //Allocating some memory
    
    vector <Mixer_unit> Mixer(50);
    
    vector <S_unit> Splitter(50);
    
    vector <PS_unit> PhaseSplitter(50);
    
    vector <Stream_class> Stream(150);
    ...
    int reactor_count = 0;          //Unit and stream counters
    int mixer_count = 0;
    int splitter_count = 0;
    int phasesplitter_count = 0;
    int stream_count = 0;
    double System_Pressure = 0;
	string System_Pressure_unit_of_measurement = "-";
    ...
    drawing.open("NetworkMap_draw",ios::out);
    
    if(drawing.fail()) //Checking drawing opening
    {   
        cout << "Error while opening NetworkMap_draw\n" ;
        return 1;
    }
...

    {
        ...
        {
            if (Splitted_line.at(0) == Main_keyword.at(5))
			{
            ...
            if (Splitted_line.at(0) == Main_keyword.at(0))       //getting REACTOR information          
...
//                                    }
    ...
    system(" && dot -Tps NetworkMap_draw -o NetworkMap.ps ");
